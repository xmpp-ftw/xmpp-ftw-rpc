...
Rpc.prototype._handleParams = function(results, params) {
    ...
    params.getChildren('param').forEach(function(param) {
       results.push(self._parseParam(param))
    })
}

Rpc.prototype._parseParam = function(param) {
    var type = param.getChild('value').children[0]
    var result = { type: type.getName() }
    switch (type.getName()) {
       case 'struct':
           break
       case 'array':
           //result.value = this._parseParam(type.getChild('
           break
       case 'boolean':
           result.value = !!(type.getText())
           break
        case 'i4':
            result.value = parseInt(type.getText())
            break
        case 'int':
            result.value = parseInt(type.getText())
        case 'double':
